...
export class BackgroundFileChunkSender {
  ...
  constructor(chunkSize: number = 35 * 1024 * 1024) {
    this.chunkSize = chunkSize // Default to 35MB chunks ish
  }
...
export class BackgroundFileChunkReceiver {
  ...
  private listener: (blob: Blob, tabId: number, fileType: string, messageId: string) => Promise<void>
  private tabId: number = -1
  private messageId: string
  private url: string = ''

  constructor(listener: (blob: Blob, tabId: number, fileType: string, messageId: string) => Promise<void>) {
  ...
  private async assembleFile() {
    ...
    this.listener(completeFile, this.tabId, this.fileType, this.messageId)
    this.fileChunks = []
  }
}
